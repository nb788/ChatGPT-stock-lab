...
def main():
    ...
    meta=json.loads((DATA/'build_metadata.json').read_text())
    bulk=pd.Timestamp(meta['built_at_utc'])
    if bulk.tzinfo is None: bulk=bulk.tz_localize('UTC')
    else: bulk=bulk.tz_convert('UTC')
    bycik=cur.groupby('cik').symbol.apply(list).to_dict(); filings=[]; api_fail=[]
    for c in sorted(set(cur.cik)):
        try:
            obj=get_json(f'https://data.sec.gov/submissions/CIK{c}.json')
            df=pd.DataFrame(base.subrows(obj,c))
            if len(df):
    ...
    for _,r in cur.iterrows():
        ...
        for (acc,ats),g in cf.groupby(['accn','acceptance_datetime'],dropna=False):
            ...
            if val is None and len(syms)==1:
                ...
                if len(vals)==1: val=float(vals[0]); method='ONE_CURRENT_TICKER_ONE_VALUE'; mem='|'.join(sorted(set(g.dimension_members.fillna('').astype(str))))
    ...
    ev=pd.DataFrame(events); ev.to_csv(DATA/'sec_live_share_events.csv',index=False)
    affected=sorted(set(fd.cik.astype(str))) if len(fd) else []
    summary={'checked_at_utc':datetime.now(timezone.utc).isoformat(),'bulk_built_at_utc':bulk.isoformat(),'current_ciks_checked':int(cur.cik.nunique()),'api_failures':int(len(api_fail)),'new_periodic_filings_since_bulk':int(len(fd)),'affected_ciks':affected,'filing_fetch_failures':int(len(fetch_fail)),'quarantined_fact_rows':int(len(bad)),'mapped_live_share_events':int(len(ev)),'production_rule':'Combine valid live events with nightly event history by acceptance time; any affected filing that cannot be fetched/mapped makes its ticker ineligible for the close screen.'}
